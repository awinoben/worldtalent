...
//    return;
    ...
    $query = "SELECT * FROM `base_standards` WHERE `Code`='$code' AND `Grp`='$group'";
    ...
    if ($count == 0) {
        ...
        $registerQuery = "INSERT INTO `base_standards`(`Code`, `Grp`,`Description`) 
		                  VALUES ('$code','$group','$description')";
        try {
            $db->query($registerQuery);

            echo "Standard submitted successfully!!";  // successful
