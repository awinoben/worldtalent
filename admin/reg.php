...
// Validation for allowed extensions .in_array() function searches an array for a specific value.
    if((!in_array($extension,$allowed_extensions)) && (!in_array($extresume,$allowed_doc)))
    {
...
// Code for move image and document into directory
        ...
        move_uploaded_file($_FILES["resume"]["tmp_name"],"resumes/".$resumedoc);
        ...
        if ($userCount == 0) {
            ...
            $registerQuery = "INSERT INTO `candidates`(`firstName`, `middleName`,`lastName`,`username`,`dob`,`age`,`gender`,`email`,`phone`,`country`,`region`,`clinicManager`,`password`,`photo`,`resume`,`assessImportance`,`profile`,`video`) 
		                  VALUES ('$fname','$mname','$lname','$username','$dob','$age','$gender','$email','$phone','$country','$region','$manager','$passwordHash','$imgnewfile','$resumedoc','$importance','$profile','$video')";
            try {
                $db->query($registerQuery);

                echo "candidate registered successfully!!";  // successful
                //redirect to registration page
                header('location:regCandidate.php');
